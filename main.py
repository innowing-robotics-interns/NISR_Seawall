...
# ─────────────────────────────────────────────────────────────────────────────
#  ADAPTIVE CUBE-ATLAS PIPELINE  (--adaptive)
#
#  Phase 1 (pretrain): fit the atlas DIRECTLY to the reference box via pointwise
#  MSE — F(leaf, uv) ≈ cube_xyz(leaf, uv). No subdivision. This validates the
#  data structure: the seam check must report ~1e-6 gaps and the exported mesh
#  must be a clean closed box.
#
#  Phase 2 (training): fit the target point cloud with global Chamfer (+ optional
#  Dirichlet tangent and/or SVD regularization). Every --subdiv_every epochs the
#  per-patch distortion is measured; leaves above --subdiv_threshold are
#  subdivided until --subdiv_max_depth.
# ─────────────────────────────────────────────────────────────────────────────

# Defaults that differ from the fixed-grid pipeline. Applied via
# parser.set_defaults() when --adaptive is given, so anything the user types
# explicitly on the command line still wins.
ADAPTIVE_DEFAULTS = {
    'shape': 'box',
    'result_dir': 'logs/adaptive',
    'N': 100000,
    'mesh_res': 40,
    'log_every': 100,
...
def main():
    # Stage 1: peek at --adaptive so the shared options can carry the adaptive
    # pipeline's defaults (they differ from the fixed-grid ones — e.g.
    # M_per_patch 128 vs 4096). Explicit command-line values still win.
    pre = argparse.ArgumentParser(add_help=False)
    pre.add_argument('--adaptive', action='store_true')
    ...
    parser.add_argument('--adaptive', action='store_true', default=False,
                        help='Use the ADAPTIVE quadtree cube atlas instead of the fixed '
                             'patch grid: box pretraining, then Chamfer fitting with '
                             'distortion-driven subdivision (see the "adaptive cube atlas" '
                             'group). Ignores --atlas_mode/--n_patches and every '
                             'correspondence/DDF option, and changes the defaults of the '
                             'shared options it does use.')
    ...
    # Adaptive quadtree cube atlas (only used with --adaptive)
    adaptive_group = parser.add_argument_group('adaptive cube atlas (--adaptive)')
    adaptive_group.add_argument('--base_subdivisions', type=int, default=1,
                                help='[Adaptive] Uniform subdivisions applied at construction '
                                     '(1 → 4 patches per face = 24 leaves)')
    adaptive_group.add_argument('--pretrain_only', action='store_true',
                                help='[Adaptive] Stop after box pretraining (structure check)')
    adaptive_group.add_argument('--skip_pretrain', action='store_true',
                                help='[Adaptive] Skip box pretraining entirely')
    ...
    # Subdivision policy
    subdiv_group = parser.add_argument_group('subdivision policy (--adaptive)')
    subdiv_group.add_argument('--subdiv_threshold', type=float, default=0.3,
    ...
    # Stage 2: swap in the adaptive defaults BEFORE parsing, so anything the
    # user typed explicitly still overrides them.
    if pre_args.adaptive:
        parser.set_defaults(**ADAPTIVE_DEFAULTS)
